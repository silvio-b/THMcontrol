...
#################### THIS IS FOR CONTINOUS ACTION SPACES ########################
...
class ReplayBuffer():
    def __init__(self, max_size, input_shape, n_actions):
        ...
        self.terminal_memory = np.zeros(self.mem_size, dtype=np.bool) #this is to store the DONE flag for terminal state
    ...
    def sample_buffer(self, batch_size):                                  ### BUFFER REPLAY ###
...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, n_actions, fc1_dims=256, fc2_dims=256,
        ...
        self.fc1 = nn.Linear(self.input_dims[0]+n_actions, self.fc1_dims)    # The Critic evaluates the value of state-action pair
...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, max_action, fc1_dims=256,
        ...
        self.reparam_noise = 1e-6                       #Prevent the occuring of values difficult to handle (1/0)

        self.fc1 = nn.Linear(*self.input_dims, self.fc1_dims)
        self.fc2 = nn.Linear(self.fc1_dims, self.fc2_dims)
        self.mu = nn.Linear(self.fc2_dims, self.n_actions)              ############_ They look the same. What's the difference? @154
    ...
    def sample_normal(self, state, reparameterize=True):
        ...
        if reparameterize:
            actions = probabilities.rsample()                                   # actions: These are the actual probabilities
        else:
            actions = probabilities.sample()

        action = T.tanh(actions)*T.tensor(self.max_action).to(self.device)      #_ action: This is the hyperbolic transformed probabilities
